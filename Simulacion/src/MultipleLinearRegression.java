public class MultipleLinearRegression extends  DataB {
    ...
    public static void metodo2() {
       ...
       //    matriz1[i][3] = datosxy.get(i).getD();
        ...
        // Limita el resultado a 4 filas y 1 columna
        double[][] resultadoLimitado = new double[columnas][1];
        for (int i = 0; i < columnas; i++) {
            resultadoLimitado[i][0] = resultado[i][0];
        }
        System.out.println();
        mostrarMatriz(resultadoLimitado);

    double[][] resultado3 = new double [columnas][1];
    for (int i=0; i < Resultado1.length;i++){
        for (int j=0;j< resultadoLimitado[0].length;j++){
            double suma = 0;
            for (int k=0; k< Resultado1[0].length; k++){
                suma+= Resultado1[i][k]*resultadoLimitado[k][j];
